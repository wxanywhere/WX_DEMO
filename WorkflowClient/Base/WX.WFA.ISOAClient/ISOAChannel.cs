...
namespace WX.WFA.ISOAClient
{
    public static class ISOAChannel
    {
        ...
        static ISOAChannel()
        {
            YDTF.ORMExpand.Common.SysHeader = new VO_SYS_HEAD(0, string.Empty);
            YDTF.ORMExpand.Common.SysError = new VO_SYS_ERROR();
            YDTF.ORMExpand.Common.EntityAssemblyName = typeof(VO_SYS_ERROR).Assembly.FullName; //"WX.WFA.VO, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null"; //Assembly.GetExecutingAssembly().FullName;
        }
        ...
        public static T CallService<T>(string serviceName, params object[] inputValues)
        {
            ...
            {
                Assembly assembly = Assembly.Load(YDTF.ORMExpand.Common.ORMAssemblyName);
